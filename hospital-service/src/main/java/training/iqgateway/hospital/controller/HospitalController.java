...
public class HospitalController {
	...
	public ResponseEntity<?> getHospitalByEmail(@PathVariable String email){
		Hospital byEmail = hospitalService.findByEmail(email);
...
//		restTemplate.put(updateHospitalURI, hospital);
	...
	public ResponseEntity<?> requestMultiNurse(@RequestBody List<NurseRequest> multiNurseRequestDetails) {
		...
		for(NurseRequest nurseRequest : multiNurseRequestDetails) {
			System.out.println("Nurse Request Details: " + nurseRequest);
			
			requestSingleNurse(nurseRequest);
		}
...
System.out.println("Inside requestSingleNurse method");
		NurseResponseDTO requestSingleNurse = null;

		try {
			System.out.println("before calling requestSingleNurse");
			requestSingleNurse = hospitalService.requestSingleNurse(singleNurseRequestDetails);
			
			System.out.println("after calling requestSingleNurse");
			...
			if (requestSingleNurse != null) {
				...
				System.out.println("I am here after calling createHospitalNurse");
	...
	@PutMapping("/cancelSingleNurseRequest")
	public ResponseEntity<?> cancelSingleNurseRequest(@RequestBody CancelRequestDetails cancelSingleRequestDetails) {
		...
		if(hospitalNurseService.deleteHospitalNurseRequest(cancelSingleRequestDetails.getStaffId(), cancelSingleRequestDetails.getFrom(),cancelSingleRequestDetails.getTo(), cancelSingleRequestDetails.getHospitalId())!=null) {
			try {
				...
				return new ResponseEntity<>("Could not cancel nurse request: " + e.getMessage(), HttpStatus.NOT_FOUND);
			}
		}
