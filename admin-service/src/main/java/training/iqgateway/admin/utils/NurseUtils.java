...
import training.iqgateway.admin.dto.NurseResponseDTO.LeaveDetail;
...
Nurse.Address address = nurse.getAddress();
		...
		nurseResponseDTO.setYearOfExperience(nurse.getYearOfExperience());
...
//		
		...
		List<NurseResponseDTO.WorkingHour> nurseResponseWorkingHours = nurseEntityWorkingHours.stream()
				...
				.map(workingHour -> new NurseResponseDTO.WorkingHour(workingHour.getDate(), workingHour.getFrom().toString(), 
		...
		List<NurseResponseDTO.WorkSchedule> nurseResponseWorkingSchedules = nurseEntityWorkSchedules.stream()
				...
				.map(workingSchedule -> new NurseResponseDTO.WorkSchedule(workingSchedule.getDate(), workingSchedule.getFrom().toString(), 
